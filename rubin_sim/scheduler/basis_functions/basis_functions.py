...
class BaseBasisFunction:
    ...
    def add_observation(self, observation, indx=None):
        ...
        """
        for feature in self.survey_features:
            self.survey_features[feature].add_observation(
                observation, indx=indx)
        if self.update_on_newobs:
            self.recalc = True
...
class FilterDistBasisFunction(BaseBasisFunction):
    ...
    def __init__(self, filtername="r"):
        ...
        self.survey_features["n_obs_count_all"] = features.NObsCount(
            filtername=None)
        # Count in filter
...
class NObsPerYearBasisFunction(BaseBasisFunction):
    ...
    def __init__(
        ...
        self.n_obs = n_obs
        self.season = season
        self.season_start_hour = (season_start_hour) * \
            np.pi / 12.0  # To radians
    ...
    def _calc_value(self, conditions, indx=None):
        ...
        behind_pix = np.where(
            (conditions.mjd - self.survey_features["last_n_mjds"].feature[0]) > self.season)
        result[behind_pix] = 1
        ...
        # ok, now
        relative_ra[
            np.where(IntRounded(relative_ra) > IntRounded(
                self.season_end_hour - self.season_start_hour))
        ] = 0
...
class NGoodSeeingBasisFunction(BaseBasisFunction):
    ...
    def _calc_value(self, conditions, indx=None):
        ...
        self.survey_features["N_good_seeing"].season_update(
...
class AvoidLongGapsBasisFunction(BaseBasisFunction):
    ...
    def __init__(
        ...
        super(AvoidLongGapsBasisFunction, self).__init__(
            nside=nside, filtername=filtername)
    ...
    def _calc_value(self, conditions, indx=None):
        ...
        in_range = np.where((gap > self.min_gap) & (
            gap < self.max_gap) & (self.footprint > 0))
        result[in_range] = 1
...
class TargetMapBasisFunction(BaseBasisFunction):
    ...
    def _calc_value(self, conditions, indx=None):
        ...
        goal_n = self.target_map[indx] * \
            self.survey_features["n_obs_count_all"].feature * self.norm_factor
...
class NObsHighAmBasisFunction(BaseBasisFunction):
    ...
    def __init__(
        ...
        super(NObsHighAmBasisFunction, self).__init__(
            nside=nside, filtername=filtername)
        self.footprint = footprint
        self.out_footprint = np.where((footprint == 0) | np.isnan(footprint))
    ...
    def _calc_value(self, conditions, indx=None):
        ...
        behind_pix = np.where(
            (
                IntRounded(conditions.mjd -
                           self.survey_features["last_n_mjds"].feature[0])
...
class CadenceInSeasonBasisFunction(BaseBasisFunction):
    ...
    def __init__(self, drive_map, filtername="griz", season_span=2.5, cadence=2.5, nside=None):
        super(CadenceInSeasonBasisFunction, self).__init__(
            nside=nside, filtername=filtername)
    ...
    def _calc_value(self, conditions, indx=None):
        ...
        days_lag = conditions.mjd - \
            self.survey_features["last_observed"].feature
...
class SeasonCoverageBasisFunction(BaseBasisFunction):
    ...
    def __init__(
        ...
        super(SeasonCoverageBasisFunction, self).__init__(
            nside=nside, filtername=filtername)
    ...
    def _calc_value(self, conditions, indx=None):
        ...
        season = utils.season_calc(
            conditions.night, offset=self.offset, floor=False)
...
class FootprintNvisBasisFunction(BaseBasisFunction):
    ...
    def __init__(
        ...
        super(FootprintNvisBasisFunction, self).__init__(
            nside=nside, filtername=filtername)
        self.footprint = footprint
        self.nvis = nvis
    ...
    def _calc_value(self, conditions, indx=None):
        result = self.result.copy()
        diff = IntRounded(self.footprint * self.nvis -
                          self.survey_features["n_obs"].feature)
...
class ThirdObservationBasisFunction(BaseBasisFunction):
    ...
    def __init__(self, nside=32, filtername1="r", filtername2="z", gap_min=40.0, gap_max=120.0):
        ...
        self.survey_features = {}
        self.survey_features["last_obs_f1"] = features.Last_observed(
            filtername=filtername1, nside=nside)
        self.survey_features["last_obs_f2"] = features.Last_observed(
            filtername=filtername2, nside=nside)
        self.result = np.empty(hp.nside2npix(self.nside))
        self.result.fill(np.nan)

    def _calc_value(self, conditions, indx=None):
        result = self.result.copy()
        d1 = IntRounded(conditions.mjd -
                        self.survey_features["last_obs_f1"].feature)
        d2 = IntRounded(conditions.mjd -
                        self.survey_features["last_obs_f2"].feature)
...
class AvoidFastRevists(BaseBasisFunction):
    ...
    def __init__(self, filtername="r", nside=None, gap_min=25.0, penalty_val=np.nan):
        super(AvoidFastRevists, self).__init__(
            nside=nside, filtername=filtername)
    ...
    def _calc_value(self, conditions, indx=None):
        ...
        diff = IntRounded(
            conditions.mjd - self.survey_features["Last_observed"].feature[indx])
...
class NearSunTwilightBasisFunction(BaseBasisFunction):
    ...
    def _calc_value(self, conditions, indx=None):
        ...
        result[good_pix] = conditions.airmass[good_pix] / \
            self.max_airmass.initial
        return result
...
class GoalStrictFilterBasisFunction(BaseBasisFunction):
    ...
    def __init__(
        ...
        super(GoalStrictFilterBasisFunction, self).__init__(
            filtername=filtername)
    ...
    def check_feasibility(self, conditions):
        ...
        # Has enough time past?
        lag = conditions.mjd - \
            self.survey_features["Last_filter_change"].feature["mjd"]
...
class AggressiveSlewtimeBasisFunction(BaseBasisFunction):
    ...
    def __init__(self, max_time=135.0, order=1.0, hard_max=None, filtername="r", nside=None):
        super(AggressiveSlewtimeBasisFunction, self).__init__(
            nside=nside, filtername=filtername)
    ...
    def _calc_value(self, conditions, indx=None):
        ...
        if conditions.current_filter != self.filtername:
            ...
            if np.size(self.condition_features["slewtime"].feature) > 1:
                ...
                good = np.where(np.bitwise_and(
                    conditions.slewtime > 0.0, conditions.slewtime < self.maxtime))
                result[good] = (
                    (self.maxtime - conditions.slewtime[good]) / self.maxtime) ** self.order
...
class SkybrightnessLimitBasisFunction(BaseBasisFunction):
    ...
    def __init__(self, nside=None, filtername="r", sbmin=20.0, sbmax=30.0):
        super(SkybrightnessLimitBasisFunction, self).__init__(
            nside=nside, filtername=filtername)
    ...
    def _calc_value(self, conditions, indx=None):
        ...
        good = np.where(
            np.bitwise_and(
                IntRounded(
                    conditions.skybrightness[self.filtername]) > self.min,
                IntRounded(
...
class CablewrapUnwrapBasisFunction(BaseBasisFunction):
    ...
    def _calc_value(self, conditions, indx=None):
        ...
        unseen = np.where(np.bitwise_or(
            conditions.alt < self.min_alt, conditions.alt > self.max_alt))
        ...
        if (
            ...
            self.active and self.unwrap_direction == -
                1 and current_abs_rad < self.max_az - self.unwrap_until
        ):
            self.active = False
            self.unwrap_direction = 0.0
            self.activation_time = None
        ...
        norm_az_rad = np.divmod(
            conditions.az - min_abs_rad, TWOPI)[1] + min_abs_rad
        ...
        unwrap_current_abs_rad = current_abs_rad - (
            np.abs(self.delta_unwrap) if self.unwrap_direction > 0 else -
            np.abs(self.delta_unwrap)
        )
        unwrap_distance_rad = divmod(
            norm_az_rad - unwrap_current_abs_rad, TWOPI)[1]
        ...
        result = (1.0 - unwrap_distance_rad /
                  np.max(unwrap_distance_rad)) ** 2.0
        result[mask] = 0.0
        result[unseen] = np.nan
...
class CadenceEnhanceBasisFunction(BaseBasisFunction):
    ...
    def __init__(
        ...
        super(CadenceEnhanceBasisFunction, self).__init__(
            nside=nside, filtername=filtername)
    ...
    def _calc_value(self, conditions, indx=None):
        ...
        if np.size(ind) == 0:
            ...
            mjd_diff = conditions.mjd - \
                self.survey_features["last_observed"].feature[ind]
            to_supress = np.where(
...
class CadenceEnhanceTrapezoidBasisFunction(BaseBasisFunction):
    ...
    def __init__(
        ...
        super(CadenceEnhanceTrapezoidBasisFunction, self).__init__(
            nside=nside, filtername=filtername)
    ...
    def suppress_enhance(self, x):
        result = x * 0
        result -= trapezoid(x, self.delay_amp, self.delay_peak,
                            self.delay_width, self.delay_slope)
        result += trapezoid(
            x,
            self.enhance_amp,
            self.enhance_peak,
...
class DecModuloBasisFunction(BaseBasisFunction):
    ...
    def __init__(self, nside=None, dec_limits=None, out_of_bounds_val=-1.0):
        ...
        if dec_limits is None:
            self.dec_limits = np.radians(
                [[-90.0, -32.8], [-32.8, -12.0], [-12.0, 35.0]])
        else:
...
class GoodSeeingBasisFunction(BaseBasisFunction):
    ...
    def __init__(
        ...
        self.survey_features = {}
        self.survey_features["coadd_depth_all"] = features.Coadded_depth(
            filtername=filtername, nside=nside)
    ...
    def _calc_value(self, conditions, **kwargs):
        ...
        diff = (
            self.survey_features["coadd_depth_all"].feature -
            self.survey_features["coadd_depth_good"].feature
        )
...
class LimitRepeatBasisFunction(BaseBasisFunction):
    ...
    def _calc_value(self, conditions, **kwargs):
        result = self.result.copy()
        good_pix = np.where(
            self.survey_features["n_obs"].feature >= self.n_limit)[0]
        result[good_pix] = 1
...
class ObservedTwiceBasisFunction(BaseBasisFunction):
    ...
    def _calc_value(self, conditions, **kwargs):
        result = self.result.copy()
        good_pix = np.where(
            (self.survey_features["n_obs_infilt"].feature >=
             self.n_obs_in_filt_needed)
...
class VisitGap(BaseBasisFunction):
    ...
    def check_feasibility(self, conditions):
        ...
        after_gap = [conditions.mjd - last_observed >
                     self.gap for last_observed in notes_last_observed]
...
class AvoidDirectWind(BaseBasisFunction):
    ...
    def _calc_value(self, conditions, indx=None):
        ...
        wind_pressure = conditions.wind_speed * \
            np.cos(conditions.az - conditions.wind_direction)
...
class BalanceVisits(BaseBasisFunction):
    ...
    def __init__(self, nobs_reference, note_survey, note_interest, nside=None):
        ...
        self.survey_features = {}
        self.survey_features["n_obs_survey"] = features.NObsSurvey(
            note=note_survey)
...
class AirmassDistBasisFunction(BaseBasisFunction):
    """Reward airmasses that have been observed less than others.
    """

    def __init__(self, nside=None, airmass_bin_edges=None):
        super().__init__(nside=nside)
        if airmass_bin_edges is None:
            self.airmass_bin_edges = np.arange(1.0, 2.4, 0.1)
        else:
            self.airmass_bin_edges = airmass_bin_edges

        self.survey_features = {}
        self.survey_features["n_airmass"] = features.AirmassBinsFeature(
            bin_edges=self.airmass_bin_edges)
        self.result = np.zeros(hp.nside2npix(nside))
    ...
    def _calc_value(self, conditions, indx=None):
        ...
        indx = np.searchsorted(self.airmass_bin_edges,
                               conditions.airmass[in_range]) - 1
